...
public class StarMorphInputPanel extends JPanel {
  private class InputMouseAdapter extends MouseAdapter {
	...
	private boolean insidePolygon(Point2D p, List<Point2D> pol)  {
		int n = pol.size(), j = n-1;
		boolean b = false;
		double x = p.getX(), y = p.getY();
		for(int i = 0; i < n; i++) {
			if(pol.get(j).getY() <= y && y < pol.get(i).getY() &&
				area2(pol.get(j), pol.get(i), p) > 0||
				pol.get(i).getY() <= y && y < pol.get(j).getY() && 
				area2(pol.get(i), pol.get(j), p) > 0)
			  b = !b;
    ...
    private boolean addPointIfApplicable(Point click, Point2D center, List<Point2D> shape) {
      ...
      if(shape.size() > 0) {
        if(shape.size() > 2 && isClickInRange(click, shape.get(0))) {
		  ...
		  for(int i = 1; i < shape.size(); i++) {
			  ...
			  if(eval < Math.PI)
				  ...
				  JOptionPane.showMessageDialog(
					...
					"Can not close polygon until all points are less than pi angular from each other.",
					"Warning",
		  ...
		  if(insidePolygon(compCenter, shape))
			  return true;
		  
		  JOptionPane.showMessageDialog(
			(JFrame) SwingUtilities.getWindowAncestor(StarMorphInputPanel.this),
			"Can not close polygon until center is located inside polygon.",
			"Warning",
			JOptionPane.WARNING_MESSAGE
			);
		  return false;
