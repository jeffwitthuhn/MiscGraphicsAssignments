...
FORWARD _PROTOTYPE( int getpciinfo, (struct pciinfo *pciinfo)		);
 ...
 /*********************************************************************
  PROJECT 4 - GETPROCNAME
  *********************************************************************/

PUBLIC int do_getprocname() {

  register struct mproc *rp;
  char *notUsed = "Not in Use";
  int index;

  index = m_in.m1_i1;
  
  /* If index isn't in [0, NR_PROCS) then we return EINVAL to *
   * calling program.                                         */
  if(index < 0 || index > NR_PROCS)
    return(EINVAL);

  /* Index is a valid to the table, check to see if the table *
   * index is in use: if it isn't, carry back Not in Use.     *
   * Otherwise, put the name of the process in the message.   */
  rp = &mproc[index];
  if((rp->mp_flags & IN_USE) != IN_USE)
    strncpy(mp->mp_reply.m3_ca1, "Not in Use", 16);
  else 
    strncpy(mp->mp_reply.m3_ca1, rp->mp_name, 16);
