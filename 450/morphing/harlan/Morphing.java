...
public class Morphing {
	public static void sortVertecies(List<Point2D> vertices) {
    Collections.sort(vertices, new Comparator<Point2D>() {
       @Override
	...
	public static HashMap<Point2D, Point2D> calcMorph(List<Point2D> src, List<Point2D> trgt, Point2D centerSource, Point2D centerTarget){
		...
		System.out.println("FROM MORPHING  (");
 	    System.out.println("center source is  ("+ centerSource.getX()+", "+centerSource.getY()+")");
		for(int z=0; z<source.size(); z++)
 		 	 System.out.println("source["+ z+"] is: ("+source.get(z).getX()+", "+source.get(z).getY()+")");
 		
 		 System.out.println("center target is  ("+ centerTarget.getX()+", "+centerTarget.getY()+")");
 		 for(int z=0; z<target.size(); z++)
 		 	 System.out.println("target["+ i+"] is: ("+target.get(z).getX()+", "+target.get(z).getY()+")");

 		int sizeS=source.size(); 
		int sizeT=target.size();
		sortVertecies(source);
      	sortVertecies(target);
